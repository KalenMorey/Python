...
class Fileapp(Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.label1 = Label(master, text = 'Last file transfered at')
        ...
        self.button1 = Button(master,text = "Browse",cursor='contextualmenuarrow', command = lambda: self.get_directory(0)) # Choose Source folder 
        self.button1.grid(row=3, column=3, sticky = E, padx = 10, pady = 10)

        self.button2 = Button(master,text = "Browse",cursor ='contextualmenuarrow', command = lambda: self.get_directory(1)) # Choose Destination folder 
        self.button2.grid(row=4, column=3, sticky = N, padx = 10, pady=10) 

        self.button3 = Button(master, text= "Move files", cursor = "center_ptr", command = lambda: self.findfiles(3)) # Move Files 
        self.button3.grid(row=5,column=2,padx = 10, pady = 10, sticky=W)

        self.button4 = Button(master, text= "Undo",cursor = "center_ptr", command = lambda: self.findfiles(4)) # Move Files 
